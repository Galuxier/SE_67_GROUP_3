...
export default function CartItem({ shopId, item, onRemoveItem, onUpdateQuantity, onSelectItem, selected }) {
  ...
  const handleIncrease = () => onUpdateQuantity(shopId, item.variant_id, item.quantity + 1);
  const handleDecrease = () => {
    if (item.quantity > 1) onUpdateQuantity(shopId, item.variant_id, item.quantity - 1);
  };
  const handleRemove = () => onRemoveItem(shopId, item.variant_id);
  const handleCheckboxChange = (e) => onSelectItem(shopId, item.variant_id, e.target.checked);
