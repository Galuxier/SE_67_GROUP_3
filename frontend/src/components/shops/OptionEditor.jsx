...
const OptionEditor = ({ option, index, onUpdateOption, onRemoveOption, onSetMain }) => {
  ...
  return (
    <div className="p-3 border bg-background border-gray-200 rounded-lg">
      ...
      {!isEditing ? (
        <div className="flex flex-wrap gap-2">
          {option.values.map((value, valueIndex) => (
            <span key={valueIndex} className="bg-gray-100 text-text rounded-full px-3 py-1 text-sm">
              {value}
            </span>
          ))}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2">
            {editedValues.map((value, valueIndex) => (
              <div key={valueIndex} className="flex items-center bg-gray-100 rounded-full px-3 py-1">
                <input
                  type="text"
                  value={value}
                  onChange={(e) => handleEditValue(valueIndex, e.target.value)}
                  className="bg-transparent text-sm border-none outline-none w-20"
                />
                <button
                  type="button"
