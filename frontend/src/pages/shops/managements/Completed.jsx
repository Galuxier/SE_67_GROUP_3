    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const mockOrders = [
          ...
          // Add first_name and last_name to mock data if not available
          {
            _id: "67ddc96ca738e660fd2915a3",
            user_id: "67ddc96ca738e660fd2915a3",
        ...
        // เรียก API จริง
        const response = await getOrdersByShopId(shopId, "completed");
        console.log("API Response:", response);

        // ตรวจสอบโครงสร้างข้อมูลจาก API
        let ordersData = Array.isArray(response) ? response : response.data || [];

        // กรอง ordersData ให้เหลือเฉพาะ status เป็น "pending"
        ordersData = ordersData.filter(
          (order) => order.status.toLowerCase() === "completed"
        );

        // ถ้า API ส่งข้อมูลกลับมาเป็น array ว่าง ให้ใช้ mock data ที่กรองแล้ว
        let finalOrders = [];
        if (ordersData.length === 0) {
          console.log("No pending orders from API, using mock data instead");
          const filteredMockOrders = mockOrders.filter(
            (order) => order.status.toLowerCase() === "completed"
          );
          finalOrders = filteredMockOrders;
        } else {
        ...
        const userPromises = finalOrders.map(async (order) => {
          try {
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-50 dark:bg-background">
      <div className="justify-center">
        <h1 className="text-5xl text-center mb-6 font-bold">Processing Orders</h1>
                  ...
                  <tr key={order._id} className="hover:bg-bar/50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col items-center space-y-2">
                        <img
                          src={
                            userData[order.user_id]?.profilePicture ||
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-text text-center">
                        {userData[order.user_id]?.firstName} {userData[order.user_id]?.lastName}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-text text-center">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-text text-center">
                        {order.items.length > 0 && order.items[0].variant_id
                          ? order.items[0].variant_id.toString()
                          : "-"}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-text text-center">{order.total_price}</div>
                    </td>
