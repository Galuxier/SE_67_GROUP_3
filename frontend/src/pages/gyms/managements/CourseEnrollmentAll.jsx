  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 1. Get all orders (using mock data if API fails)
        let orders = [];
        try {
          orders = await getUserOrders('current_user_id'); // Need actual user ID
          console.log('Orders from API:', orders);
        } catch (apiError) {
          console.warn('Using mock orders data due to API error:', apiError);
          orders = mockEnrollments;
        }

        // 2. Filter only course orders
        const courseOrders = orders.filter(
          order => order.order_type === 'course' || 
                  (order.items && order.items.some(item => item.ref_model === 'Course'))
        );

        // 3. Enrich with user and course data
        const enrichedEnrollments = await Promise.all(
          courseOrders.map(async order => {
            try {
              // Get user data
              let user = {};
              try {
              ...
              // Get course data (only for the first item)
              let course = {};
        ...
        setEnrollments(enrichedEnrollments.filter(Boolean));
      } catch (error) {
