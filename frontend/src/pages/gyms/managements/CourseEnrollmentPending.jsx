    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          console.log('Pending Orders from API:', orders);
        } catch (apiError) {
          console.warn('Using mock orders data due to API error:', apiError);
          orders = mockEnrollments.filter(order => order.status === 'pending');
        }
  
        // 2. กรองเฉพาะ orders ประเภท course และสถานะ pending
        const pendingCourseOrders = orders.filter(
        ...
        const enrichedPendingOrders = await Promise.all(
          pendingCourseOrders.map(async order => {
            try {
              // ดึงข้อมูลผู้ใช้
              let user = {};
              try {
        ...
        setEnrollments(enrichedPendingOrders.filter(Boolean));
      } catch (error) {
        console.error('Failed to fetch pending enrollments:', error);
        setError('Failed to load pending enrollments. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
