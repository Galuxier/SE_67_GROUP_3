...
export const getTrainersNotInGymController = async (req: Request, res: Response) => {
  try {
    ...
    const trainers = await UserService.getTrainersNotInGym(gymId);
    ...
    console.error('Error fetching trainers not in gym:', err);
    res.status(500).json({ 
      success: false, 
      message: 'Error fetching trainers not in gym', 
      error: err 
    });
  }
};
